...
function getLocalStoragePreset() {
  if (typeof window !== "undefined") {
    const localStorageValue = localStorage.getItem("hub-preset");
...
const colorPresetsAtomWithPersistence = atom(
  ...
  (get, set, newStorage: any) => {
    ...
    localStorage.setItem("hub-preset", JSON.stringify(newStorage));
  }
);
...
const colorPresetNameAtom = atom(
  typeof window !== "undefined"
    ? localStorage.getItem("hub-preset-name")
    : DEFAULT_PRESET_COLOR_NAME
);
...
const colorPresetNameAtomWithPersistence = atom(
  ...
  (get, set, newStorage: any) => {
    ...
    localStorage.setItem("hub-preset-name", newStorage);
  }
);
